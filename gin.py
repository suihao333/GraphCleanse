...
class Encoder(torch.nn.Module):
    ...
    def get_embeddings(self, loader):
        ...
        with torch.no_grad():
            for data in loader:
                subgraph_id = data[2]
                gid.append(subgraph_id.cpu().numpy())
                data = data[0]   
                data.to(device)
                x, edge_index, batch = data.x, data.edge_index, data.batch
                if x is None:
                    x = torch.ones((batch.shape[0],1)).to(device)
                x, nodeemb = self.forward(x, edge_index, batch)
                ret.append(x.cpu().numpy())
                node_emb.append(nodeemb.cpu().numpy())
                # adj.append(edge_index.cpu().numpy())
                y.append(data.y.cpu().numpy())
        node_emb = np.concatenate(node_emb, 0)
        # adj = np.concatenate(adj, 0)
        ret = np.concatenate(ret, 0)
        y = np.concatenate(y, 0)
        gid = np.concatenate(gid, 0)  
        return node_emb, ret, y, gid
